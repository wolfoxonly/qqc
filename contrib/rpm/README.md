...
The RPM spec file provided here is for QQcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://QQcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[QQcoin git repository](https://github.com/QQcoin/QQcoin) but are not part of
...
Currently only `Source10` is used, for the example `QQcoin.conf` file.
...
in the QQcoin git repository. At present only an SVG version of the QQcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the QQcoin
...
needed when 0.12.1 is released, a proper fix is already in the QQcoin git
master branch.
...
idea to build QQcoin Core against a static version of that library compiled
...
The `QQcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `QQcoin-server` package. At this point, most RPM based
...
The `QQcoin-server` package should have SELinux support. How to properly do
...
QQcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/QQcoin/). He has been
packaging QQcoin for Fedora at least since 2012.
...
4. I renamed the `QQcoin` package that contains the Qt GUI to `QQcoin-core` as
that appears to be how the general population refers to it, in contrast to
`QQcoin-xt` or `QQcoin-classic`. I wanted to make sure the general population
