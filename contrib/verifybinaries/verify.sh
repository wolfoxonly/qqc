...
###   QQcoincore.org and QQcoin.org and compares them.
...
WORKINGDIR="/tmp/QQcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://QQcoincore.org"
HOST2="https://QQcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="QQcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'QQcoin-',
...
if [ $? -ne 0 ]; then
   echo "QQcoin.org failed to provide signature file, but QQcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "QQcoin.org and QQcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the QQcoin Core binary release signing key installed?"
   fi
