#!/bin/bash
# Copyright (c) 2013 The QQcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete QQcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no QQcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old QQcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected QQcoin 0.7 datadir."
    ;;
  3)
    echo "Detected QQcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected QQcoin 0.8 datadir."
    ;;
esac
