...
boost::filesystem::path GetConfigFile()
{
    ...
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        ...
        else
        {
            ...
            try {
                const char* content = "rpcuser=QQcoinrpc\nrpcpassword=QQcoinrpcpassword";
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No QQcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override QQcoin.conf
...
// Format the subversion field according to BIP 14 spec (https://en.QQcoin.it/wiki/BIP_0014)
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "QQcoin:" << FormatVersion(QQCOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
