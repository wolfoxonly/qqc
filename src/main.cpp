...
// Copyright (c) 2009-2012 The QQcoin developers
// Copyright (c) 2011-2015 The QQcoin developers
...
const string strMessageMagic = "QQcoin1.0 Signed Message:\n";//ZXB
...
int64 GetProofOfStakeReward(int64 nCoinAge,int64 balance)//<zxb>
{
    ...
    float rate = 10*pow(0.5,year);//zxb
    printf("rate ===== %f\n", rate);
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in QQcoin
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsLimit =%d,nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "QQcoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnProofOfWorkLimit.GetCompact(),bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
// ~uint256(0) >> 32
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 15/4/2018 QQC genesis generated.";// "Matonis 15-APR-2018 Parallel Currencies And The Roadmap To Monetary Freedom";
        CTransaction txNew;
        txNew.nTime = 1523765600;//
        txNew.vin.resize(1);
        txNew.vout.resize(1);
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
        {
          ...
          printf("POW pindexPrev.nHeight===%d\n", pindexPrev->nHeight);
           ...
           {
                pblock->vtx[0].vout[0].nValue = 8239136670 * COIN;//首块产量<zxb>
           }
           else
           {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("QQcoinMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QQcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QQcoinMiner : ProcessBlock, block not accepted");
    }
...
#ifdef TESTING
void QQcoinMiner(CWallet *pwallet, bool fProofOfStake, bool fGenerateSingleBlock)
#else
void QQcoinMiner(CWallet *pwallet, bool fProofOfStake)
#endif
{
    ...
    try { loop {
        ...
        printf("Running QQcoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("QQcoinMiner terminated\n");
        throw;
    }
}

void GenerateQQcoins(bool fGenerate, CWallet* pwallet)
{
...
#ifdef TESTING
        minerThreads->create_thread(boost::bind(&QQcoinMiner, pwallet, false, false));
#else
        minerThreads->create_thread(boost::bind(&QQcoinMiner, pwallet, false));
#endif
}
